...
import static ru.practicum.request.mapper.ParticipationRequestMapper.toParticipationRequestDtoList;
...
public class ParticipationRequestServiceImpl implements ParticipationRequestService {
    ...
    public ParticipationRequestDto createParticipationRequest(Long userId, Long eventId, LocalDateTime localDateTime) {
        ...
        if (userId.equals(event.getInitiator().getId())) {
            throw new ConflictException("Запрос на участие уже отправлен");
        }

        if (!event.getState().equals(State.PUBLISHED)) {
            throw new ConflictException("Не далось опубликовать событие");
        }
        ...
        if (limit != 0) {
            ...
            if (numberOfRequests >= limit) {
                throw new ConflictException("Слишком много участников события");
            }
        }
    ...
    public ParticipationRequestDto cancelParticipationRequest(Long userId, Long requestId) {
        ...
        throw new ConflictException("Чужой запрос нельзя отменить");
    ...
    public List<ParticipationRequestDto> getAllUserEventRequests(Long eventId, Long userId) {
        ...
        if (!event.getInitiator().getId().equals(userId)) {
            throw new ConflictException("Пользователь не может просматривать запросы к событию, автором которого он не является");
        }
